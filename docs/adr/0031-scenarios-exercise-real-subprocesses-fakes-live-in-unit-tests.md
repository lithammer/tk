...
(`curl`) — are covered by `FakeRunner` unit tests and have no scenarios. Under
this policy, scenarios assert the user-visible byte contract through the real
OS boundary. Logic branches that depend on subprocess outcomes (spawn failure,
non-zero exit, stderr capture, output parsing) belong to unit tests, where the
trait seam scripts them precisely.
...
- **The binary has no injection channel.** tk-105 removed every
  process-level determinism seam (`TK_NOW`, `SOURCE_DATE_EPOCH`,
  `TK_RAND_SEED`) from the production binary because they were hidden inputs
...
- **Real gh/acli/curl in scenarios is not an option either.** Those
  ...
  version-dependent — the opposite of a frozen byte contract. Reliable sync
  scenarios would therefore need more fixture machinery.
- **The split gives each failure one meaning.** A scenario failure means the
...
- **Extend the scenario harness to inject a scripted runner.** Rejected: it
  needs a channel into the spawned binary, which tk-105 closed.
