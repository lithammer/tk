...
Use `tk done <id>` for completed work. Use `tk add -F -` for follow-ups,
deferred decisions, or context that should survive a fresh session. Use `tk
sync log` to check whether local sync state needs human attention.
