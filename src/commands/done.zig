//! `tk done` — mark one Ticket or Epic done.
...
pub const meta: cli.CommandMeta = .{
    .name = "done",
    .description = "Mark one Ticket or Epic done",
};
...
/// Parse `tk done` args, resolve the Display ID or Alias, and mark it done.
pub fn run(deps: cli.Deps, args_iter: anytype) !u8 {
    ...
    const id = res.positionals[0] orelse {
        deps.stderr.writeAll(messages.done_id_required ++ "\n") catch {};
        return 2;
    };
    ...
    const resolved = r.resolve(id, .{
        .prefix = messages.done_id_not_found_prefix,
        .suffix = messages.done_id_not_found_suffix,
    }) orelse return 1;
...
fn writeHelp(deps: cli.Deps) !void {
    try deps.stdout.writeAll(
        \\tk done - mark one Ticket or Epic done
        \\
        \\Usage:
        \\  tk done <id> [options]
        \\
...
const storage_msgs: resolver.StorageErrorMessages = .{
    .busy_retry = messages.done_store_busy_retry,
    .out_of_memory = messages.done_out_of_memory,
    .fallback = messages.done_write_failed,
};

const open_msgs: resolver.OpenMessages = .{
    .command_name = "done",
    .missing_store = messages.done_missing_store,
...
test "done: --help prints usage and exits 0" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, h.stdout(), "tk done") != null);
    try std.testing.expect(std.mem.indexOf(u8, h.stdout(), "tk done <id>") != null);
...
test "done: requires a positional id" {
    ...
    try std.testing.expectEqualStrings(messages.done_id_required ++ "\n", h.stderr());
}

test "done: reports missing store as exit 1" {
    const gpa = std.testing.allocator;
    var tmp_store = try TmpStore.init(gpa, "project");
    ...
    try std.testing.expectEqualStrings("", h.stdout());
    try std.testing.expectEqualStrings(messages.done_missing_store ++ "\n", h.stderr());
}

test "done: reports unknown id as exit 1" {
    const gpa = std.testing.allocator;
    ...
    try std.testing.expectEqualStrings(
        messages.done_id_not_found_prefix ++ "no-such-id" ++ messages.done_id_not_found_suffix ++ "\n",
        h.stderr(),
    );
}

test "done: resolves Alias and marks a local Ticket done" {
    const gpa = std.testing.allocator;
    var fixture = try StoreFixture.init(gpa);
    defer fixture.deinit(gpa);
    ...
    try std.testing.expectEqual(@as(u8, 0), try run(h.deps(), &h.iter));
    try std.testing.expectEqualStrings("Marked Ticket done: project-1 - Local Ticket\n", h.stdout());
    try std.testing.expectEqualStrings("", h.stderr());
...
test "done: marks a local Epic done" {
    const gpa = std.testing.allocator;
    var fixture = try StoreFixture.init(gpa);
    defer fixture.deinit(gpa);
    const conn = try zqlite.open(fixture.tmp_store.db_path.ptr, zqlite.OpenFlags.ReadWrite | zqlite.OpenFlags.EXResCode);
    ...
    try std.testing.expectEqual(@as(u8, 0), try run(h.deps(), &h.iter));
    try std.testing.expectEqualStrings("Marked Epic done: project-1 - Local Epic\n", h.stdout());
    try std.testing.expectEqualStrings("", h.stderr());
...
test "done: Backend Ticket success emits set_item_status Mutation" {
    const gpa = std.testing.allocator;
    var fixture = try StoreFixture.init(gpa);
    defer fixture.deinit(gpa);
    const conn = try zqlite.open(fixture.tmp_store.db_path.ptr, zqlite.OpenFlags.ReadWrite | zqlite.OpenFlags.EXResCode);
    ...
    try std.testing.expectEqual(@as(u8, 0), try run(h.deps(), &h.iter));
    try std.testing.expectEqualStrings("Marked Ticket done: GH#7 - Backend Ticket\n", h.stdout());
    ...
    try std.testing.expectEqualStrings("{\"status\":\"done\"}", row.text(1));
    const item_row = (try conn.row("select status from items where id = 'bt1'", .{})) orelse return error.ExpectedRow;
    defer item_row.deinit();
    try std.testing.expectEqualStrings("done", item_row.text(0));
}

test "done: already-done target is a no-op" {
    const gpa = std.testing.allocator;
    var fixture = try StoreFixture.init(gpa);
    defer fixture.deinit(gpa);
    const conn = try zqlite.open(fixture.tmp_store.db_path.ptr, zqlite.OpenFlags.ReadWrite | zqlite.OpenFlags.EXResCode);
...
test "done: forced write failure reports diagnostic and rolls back" {
    const gpa = std.testing.allocator;
    var fixture = try StoreFixture.init(gpa);
    defer fixture.deinit(gpa);
    const conn = try zqlite.open(fixture.tmp_store.db_path.ptr, zqlite.OpenFlags.ReadWrite | zqlite.OpenFlags.EXResCode);
    ...
    try std.testing.expectEqual(@as(u8, 1), try run(h.deps(), &h.iter));
    try std.testing.expectEqualStrings("", h.stdout());
    try std.testing.expect(std.mem.indexOf(u8, h.stderr(), messages.done_write_failed) != null);
